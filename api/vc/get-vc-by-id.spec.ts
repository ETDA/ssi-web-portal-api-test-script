...
describe('Get VC by ID', () => {
  ...
  test('Get VC by ID', async () => {
    try {
      ...
      const getVcById = await VCs.GetById(state.vcId, state.token)
      console.log('Get VC ID: ', JSON.stringify(getVcById.data, null, 2))
      expect(getVcById.status).toEqual(200)
      expect(getVcById.data).toStrictEqual(state.vc)
    } catch (err) {
  ...
  test('Get VC by ID - Send request with incorrect ID', async () => {
    try {
      const userName = USER_INFO.USER_NAME
      const password = USER_INFO.PASSWORD
