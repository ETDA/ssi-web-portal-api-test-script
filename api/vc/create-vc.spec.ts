import { ERR_REQUIRE, ID_INFO, INVALID_PARAMS, KEY_TYPE, USER_INFO } from '../consts'
...
describe('Create VC', () => {
  ...
  test('Create VC', async () => {
    try {
      ...
      expect(createVc.data.schema_name).toBe(schemaName)
      expect(createVc.data.issuer).toBe(state.firstName + ' ' + state.lastName)
      expect(createVc.data.holder).toBe(state.firstName + ' ' + state.lastName)
    } catch (err) {
  ...
  test('Create VCs - Send request with unknown signer\'s did', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const createVc = await VCs.Create(state.fullSchemaId, schemaName, schemaType, dummyDid,
        state.data.did, credentialSubject, state.token)
      console.log('Create VC: ', JSON.stringify(createVc.data, null, 2))
      expect(createVc.status).toEqual(404)
    } catch (err) {
      console.log(err.response)
  ...
  test('Create VCs - Send request with unknown holder\'s did', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const dummyDid = `did:idin:${uuidv4()}`
      const createVc = await VCs.Create(state.fullSchemaId, schemaName, schemaType, state.data.did
        , dummyDid, credentialSubject, state.token)
      console.log('Create VC: ', JSON.stringify(createVc.data, null, 2))
      expect(createVc.status).toEqual(404)
    } catch (err) {
      console.log(err.response)
  ...
  test('Create VCs - Send request with incompatible credentialSubject with schema', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const createVc = await VCs.Create(state.fullSchemaId, schemaName, schemaType, state.data.did,
        state.data.did, credentialSubject, state.token)
      console.log('Create VC: ', JSON.stringify(createVc.data, null, 2))
      expect(createVc.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      expect(err).toBeTruthy()
  ...
  test('Create VCs - Send request with incorrect credentialSchema.id', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const schemaId = `https://ssi-test.teda.th/api/schemas/${uuidv4()}/1.0.0/NationalImplementationSpecialist.json`
      const createVc = await VCs.Create(schemaId, schemaName, schemaType, state.data.did,
        state.data.did, credentialSubject, state.token)
      console.log('Create VC: ', JSON.stringify(createVc.data, null, 2))
      expect(createVc.status).toEqual(400)
    } catch (err) {
  ...
  test('Create VCs - Send request with incorrect credentialSchema.type', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const createVc = await VCs.Create(state.fullSchemaId, schemaName, schemaType + '_1', state.data.did,
        state.data.did, credentialSubject, state.token)
      console.log('Create VC: ', JSON.stringify(createVc.data, null, 2))
      expect(createVc.status).toEqual(400)
    } catch (err) {
  ...
  test('Create VCs - Send request without signer\'s did', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
  ...
  test('Create VCs - Send request without holder\'s did', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      expect(err.response.data.fields.holder.code).toBe(ERR_REQUIRE.HOLDER.CODE)
      expect(err.response.data.fields.holder.message).toBe(ERR_REQUIRE.HOLDER.MESSAGE)
    }
  })

  test('Create VCs - Send request without credentialSubject with schema', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      expect(err.response.data.code).toBe(INVALID_PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID_PARAMS.MESSAGE)
      expect(err.response.data.fields.credentialSubject.code).toBe(ERR_REQUIRE.CREDENTIAL_SUBJECT.CODE)
      expect(err.response.data.fields.credentialSubject.message).toBe(ERR_REQUIRE.CREDENTIAL_SUBJECT.MESSAGE)
    }
  })

  test('Create VCs - Send request without credentialSchema.id', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const createVc = await VCs.Create('', schemaName, schemaType, state.data.did,
        state.data.did, credentialSubject, state.token)
      console.log('Create VC: ', JSON.stringify(createVc.data, null, 2))
      expect(createVc.status).toEqual(400)
    } catch (err) {
      ...
      expect(err.response.data.code).toBe(INVALID_PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID_PARAMS.MESSAGE)
      expect(err.response.data.fields['credentialSchema.id'].code).toBe(ERR_REQUIRE.CREDENTIAL_SUBJECT_ID.CODE)
      expect(err.response.data.fields['credentialSchema.id'].message).toBe(ERR_REQUIRE.CREDENTIAL_SUBJECT_ID.MESSAGE)
    }
  })

  test('Create VCs - Send request without credentialSchema.type', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const createVc = await VCs.Create(state.fullSchemaId, schemaName, '', state.data.did,
        state.data.did, credentialSubject, state.token)
      console.log('Create VC: ', JSON.stringify(createVc.data, null, 2))
      expect(createVc.status).toEqual(400)
    } catch (err) {
      ...
      expect(err.response.data.code).toBe(INVALID_PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID_PARAMS.MESSAGE)
      expect(err.response.data.fields['credentialSchema.type'].code).toBe(ERR_REQUIRE.CREDENTIAL_SUBJECT_TYPE.CODE)
      expect(err.response.data.fields['credentialSchema.type'].message).toBe(ERR_REQUIRE.CREDENTIAL_SUBJECT_TYPE.MESSAGE)
    }
  })
})
