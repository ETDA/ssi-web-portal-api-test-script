...
describe('Get all VC', () => {
  ...
  test('Get all VC', async () => {
    try {
      ...
      const getAllVc = await VCs.Get(state.token)
      console.log('Get all VC: ', JSON.stringify(getAllVc.data, null, 2))
      expect(getAllVc.status).toEqual(200)
      expect(getAllVc.data.items[0]).toStrictEqual(state.vc)
    } catch (err) {
