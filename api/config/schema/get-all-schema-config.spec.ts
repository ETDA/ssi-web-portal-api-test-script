...
describe('Get all schema config', () => {

  const state = getInitState()
  jest.setTimeout(20000)

  test('Get all schema config', async () => {
    try {
      ...
      expect(getSchemaConfig.data.items).toEqual(
  ...
  test('Get all schema config - Send request with invalid token', async () => {
    try {
      state.token = uuidv4()
