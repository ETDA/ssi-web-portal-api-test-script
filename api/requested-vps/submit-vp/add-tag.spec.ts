...
describe('Add VP tag', () => {
  ...
  test('Add VP tag', async () => {
    try {
      ...
      const tags = ['Test', 'VC', 'Submitted']
      const addTag = await SubmitVP.AddTag(state.submittedId, tags, state.token)
      console.log('Add tag: ', JSON.stringify(addTag.data, null, 2))
      expect(addTag.status).toEqual(200)
      expect(addTag.data.tags).toBe(tags[0] + ',' + tags[1] + ',' + tags[2])
    } catch (err) {
  ...
  test('Add VP tag again after added', async () => {
    try {
      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      ...
      const addTag2 = await SubmitVP.AddTag(state.submittedId, tags, state.token)
      console.log('Add tag2: ', JSON.stringify(addTag2.data, null, 2))
      expect(addTag2.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
