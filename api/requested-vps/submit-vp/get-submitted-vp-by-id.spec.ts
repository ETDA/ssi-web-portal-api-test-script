...
describe('Get submitted VP by ID', () => {
  ...
  test('Get submitted VP by ID', async () => {
    try {
      ...
      const getSubmitById = await SubmitVP.GetSubmittedByID(state.submittedId, state.token)
      console.log('Get submitted by ID: ', JSON.stringify(getSubmitById.data, null, 2))
      expect(getSubmitById.status).toEqual(200)
      expect(getSubmitById.data.id).toBe(state.submittedId)
    } catch (err) {
