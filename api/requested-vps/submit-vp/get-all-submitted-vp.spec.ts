...
describe('Get all submitted VP', () => {
  ...
  test('Get all submitted VP', async () => {
    try {
      ...
      const getSubmitted = await SubmitVP.GetAllSubmitted(state.requestedId,state.token)
      console.log('Get submitted: ', JSON.stringify(getSubmitted.data, null, 2))
      expect(getSubmitted.status).toEqual(200)
      expect(getSubmitted.data.items[0].id).toBe(state.submittedId)
    } catch (err) {
