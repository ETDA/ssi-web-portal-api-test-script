...
describe('Get user in group', () => {

  const state = getInitState()
  jest.setTimeout(20000)

  test('Get user in group', async () => {
    try {
      ...
      const GetUserInGroup = await PortalMobile.GetUserInGroup(state.groupId,state.token)
      console.log('Get user in group: ', JSON.stringify(GetUserInGroup.data, null, 2))
      expect(GetUserInGroup.status).toEqual(200)
    } catch (err) {
